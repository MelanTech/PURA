...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='vitb_256_tbsi_32x1_1e4_lasher_15ep_sot_cls',
...
def evaluate_vit(model, template, search):
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(**{'template': template, 'search': search})
        start = time.time()
        for i in range(T_t):
            _ = model(**{'template': template, 'search': search})
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "tbsi_track":
        ...
        template = [torch.zeros(2, bs, 3, z_sz, z_sz).to(device), torch.zeros(2, bs, 3, z_sz, z_sz).to(device)]
        search = [torch.zeros(bs, 3, x_sz, x_sz).to(device), torch.zeros(bs, 3, x_sz, x_sz).to(device)]
        # transfer to device
        model = model.to(device)

        pura.replace_batchnorm(model)
        pura.configure_model(model)
