...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0,
    ...
    outputs = out_dict['score_map'].view(1, -1)

    # adapt
    # score_map = outputs['score_map']
    # out_x = score_map.sum(dim=2).view(1, -1)
    # out_y = score_map.sum(dim=1).view(1, -1)
    ...
    if fishers is not None:
        ...
        loss += ewc_loss
    if x['search'][1][ids1][ids2].size(0) != 0:
        loss.backward()
        optimizer.step()
    optimizer.zero_grad()
    return out_dict, entropys.size(0), filter_ids_1[0].size(0), updated_probs
