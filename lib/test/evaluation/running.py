...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['gtot', 'rgbt210', 'rgbt234', 'vtuav', 'lasher', ]:
        if not os.path.exists(os.path.join(tracker.results_dir)):
            os.makedirs(os.path.join(tracker.results_dir))
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        ...
        # dataset = [x for x in dataset if x.name == 'afterrain']
