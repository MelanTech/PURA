    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                ...
                if report_name == 'vtuav_test':  # zzf 每隔10个取一个
                    pred_bb = pred_bb[::10]
            else:
                if skip_missing_seq:
            ...
            # if report_name in ['vtuav_test', 'rgbt234']:  # zzf 从两种模态的anno里选择最好的效果
            #     anno_bb_2 = torch.tensor(seq.ground_truth_rect_tir)
            #     err_overlap_2, err_center_2, err_center_normalized_2, valid_frame_2 = \
            #         calc_seq_err_robust(pred_bb, anno_bb_2, seq.dataset, target_visible)
            #     err_overlap = torch.max(err_overlap, err_overlap_2)
            #     err_center = torch.min(err_center, err_center_2)
            #     err_center_normalized = torch.min(err_center_normalized, err_center_normalized_2)
            ...
            ave_success_rate_plot_overlap[seq_id, trk_id, :] = \
                (err_overlap.view(-1, 1) > threshold_set_overlap.view(1, -1)).sum(0).float() / seq_length
            ave_success_rate_plot_center[seq_id, trk_id, :] = \
                (err_center.view(-1, 1) <= threshold_set_center.view(1, -1)).sum(0).float() / seq_length
            ave_success_rate_plot_center_norm[seq_id, trk_id, :] = \
                (err_center_normalized.view(-1, 1) <= threshold_set_center_norm.view(1, -1)).sum(0).float() / seq_length
