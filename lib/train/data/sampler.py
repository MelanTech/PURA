...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
        ...
        self.num_template_frames = 3  # update zzf 采样三组模板, 输出两组
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                # template_frames, template_anno = dataset.get_front_frames(seq_id, template_frame_ids, seq_info_dict)
                ...
                if random.randint(0, 1) == 0:
                    data['template_images'][1][:3, :, :] = data['template_images'][2][:3, :, :]
                else:
                    data['template_images'][1][3:, :, :] = data['template_images'][2][3:, :, :]
                ...
                # data['template_images'] [num,6,128,128] num指模板数量,6是两种模态图片一共6个通道,前三个可见光,后三个红外
                ...
                #
                # image = de_norm(data_vi['visible']['search_images']).squeeze()
                # torchvision.utils.save_image(image, '/data/zzf/proj/TBSI/1.png')
                # print(data_vi['visible']['search_anno'])
                # print('finish')
        ...
        while not valid:
            ...
            try:
                ...
                # negative samples
                    ...
                    label = torch.zeros(1, )
                    if is_video_dataset:
                        search_frame_ids_negative = self._sample_visible_ids(visible, num_ids=1, force_invisible=True)
                        if search_frame_ids_negative is None:
                            search_frames, search_anno = self.get_one_search(dataset=dataset, seq_id=seq_id,
    ...
    def get_one_search(self, dataset=None, seq_id=None, search_frame_ids=None, seq_info_dict=None):
        if dataset is None:
            # Select a dataset
            dataset = random.choices(self.datasets, self.p_datasets)[0]
        ...
        y_max, x_max = search_frames[0].shape[0:2]
        search_anno['bbox'][0] = self.get_negtive_search_anno(search_anno, x_max, y_max)

        return search_frames, search_anno

    def get_negtive_search_anno(self, search_anno, x_max, y_max):
        anno_list = []
        x, y, w, h = search_anno['bbox'][0].tolist()
        if 0 < y - h:
            anno_list.append([x, y - h, w, h])
        if y + 2 * h < y_max:
            anno_list.append([x, y + h, w, h])
        if 0 < x - w:
            anno_list.append([x - w, y, w, h])
            # if y + 1.5 * h + 1 < y_max:
            #     anno_list.append([x - w, y + 0.5 * h, w, h])
            # if 0 < y - 0.5 * h - 1:
            #     anno_list.append([x - w, y - 0.5 * h, w, h])
        if x + 2 * w < x_max:
            anno_list.append([x + w, y, w, h])
            ...
            # if 0 < y - 0.5 * h - 1:
            #     anno_list.append([x + w, y - 0.5 * h, w, h])
        id = random.randint(0, len(anno_list) - 1)

        # # zzf 添加抖动
        # x, y, w, h = anno_list[id]
        # if 0 < x - 0.4 * w and x + 1.4 * w < x_max \
        #         and 0 < y - 0.4 * h and y + 1.4 * h < y_max:
        #     w = w + random.uniform(-w * 0.1, w * 0.1)
        #     h = h + random.uniform(-h * 0.1, h * 0.1)
        #     x = x + random.uniform(-w * 0.2, w * 0.2)
        #     y = y + random.uniform(-h * 0.2, h * 0.2)
        #     anno_list[id] = [x, y, w, h]

        return torch.tensor(anno_list[id])
