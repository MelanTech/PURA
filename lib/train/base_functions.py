...
def update_settings(settings, cfg):
    ...
    # settings.print_stats = ['Loss/total', 'Loss/giou', 'Loss/l1', 'Loss/location', 'IoU']
    settings.print_stats = ['Loss/total', 'Loss/location', 'IoU', 'Loss/scores', 'Loss/scores_tir']
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "LasHeR_train", "LasHeR_test", "VTUAV_train", "VTUAV_test", "rgbt234", "rgbt210"]
        if name == "LASOT":
        ...
        if name == "VTUAV_train":
            datasets.append(VTUAV(settings.env.vtuav_train_dir, split='train', image_loader=image_loader, modality='RGBT'))
        if name == "VTUAV_test":
            datasets.append(VTUAV(settings.env.vtuav_test_dir, split='test', image_loader=image_loader, modality='RGBT'))
        if name == "rgbt234":
            datasets.append(RGBT234(settings.env.rgbt234_dir, split='train', image_loader=image_loader))
        if name == "rgbt210":
            datasets.append(RGBT210(settings.env.rgbt210_dir, split='train', image_loader=image_loader))
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "cls_" in n and p.requires_grad]}
        ]
        if is_main_process():
            ...
            for n, p in net.named_parameters():
                if p.requires_grad:
                    print(n)
    elif cfg.TRAIN.SOT_PRETRAIN:  # =======================加载需要优化的网络===============================================
        for n, p in net.named_parameters():  # 阶段一不训练分类器
            if "cls_" in n:
                p.requires_grad = False
        param_dicts = [
            {
                "params": [p for n, p in net.named_parameters() if "backbone.tbsi" in n and p.requires_grad],
                "lr": cfg.TRAIN.LR * 1,
            },
            {
                "params": [p for n, p in net.named_parameters() if "backbone.cnn__" in n and p.requires_grad],
                "lr": cfg.TRAIN.LR * 1,
            },
            {
                "params": [p for n, p in net.named_parameters()
                           if "backbone.tbsi" not in n
                           and "backbone.cnn__" not in n
                           and p.requires_grad],
                "lr": cfg.TRAIN.LR * 0.1,
            },
        ]
