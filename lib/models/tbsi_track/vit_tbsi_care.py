...
from lib.models.my_layers.cnn_layer import CNNLayer


# from lib.models.my_layers.cnn_layer import BlockCNN


class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., current_layer=None):
        super().__init__()
        ...
        self.mask = torch.zeros([768, 768], dtype=torch.bool).cuda(device)

        # if current_layer <= -2:
        #     self.mask[:320, 320:640] = True
        #     self.mask[320:640:, :320] = True
        #
        #     self.mask[:320, 704:] = True
        #     self.mask[704:, :320] = True
        #
        #     self.mask[320:640, 640:704] = True
        #     self.mask[640:704, 640:704] = True
        #
        #     self.mask[640:704, 704:] = True
        #     self.mask[704:, 640:704] = True
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, current_layer=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, current_layer=current_layer)
...
class CNNModel(nn.Module):
    def __init__(self):
        super().__init__()
        # self.cnn_rgb_layers = CNNLayer()
        # self.cnn_tir_layers = CNNLayer()

        from lib.models.my_layers.cnn_layer import CTFusion
        # self.CT_fusion_rgb = CTFusion()
        # self.CT_fusion_tir = CTFusion()
        self.CT_fusion_rgb = nn.ModuleList()
        self.CT_fusion_tir = nn.ModuleList()
        for i in range(3):
            self.CT_fusion_rgb.append(CTFusion())
            self.CT_fusion_tir.append(CTFusion())
...
class VisionTransformerTBSI(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', num_template=1,
        ...
        """
        super().__init__()
        self.num_template = num_template  # zzf data 模板数量
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, current_layer=i)
        ...
        # self.use_cnn = True  # =================================注意！！！=================================================
        # if self.use_cnn:
        #     self.cnn__ = CNNModel()

        self.num_template_token = 64
        self.num_search_token = 256
        self.num_zx = 320
    ...
    def forward_features(self, z, x):
        ...
        # cnn_x = [tensor.clone() for tensor in x]
        # cnn_z = [tensor.clone() for tensor in z]

        x_v = self.patch_embed(x[0])
        z_v = self.patch_embed(z[0].view(-1, *z[0].shape[2:]))
        x_i = self.patch_embed(x[1])
        z_i = self.patch_embed(z[1].view(-1, *z[1].shape[2:]))

        z_v = z_v.view(self.num_template, -1, *z_v.shape[1:])
        z_i = z_i.view(self.num_template, -1, *z_i.shape[1:])
        ...
        # x_v = self.pos_drop(x_v)  # drop_rate 为 0, 注释掉
        # x_i = self.pos_drop(x_i)

        x_single = torch.cat([z_v[0], x_v, z_i[0], x_i, z_v[1], z_i[1]], dim=1)
        tbsi_index = 0
        for i, blk in enumerate(self.blocks):
            x_single = blk(x_single)
            # if self.tbsi_loc is not None and i in [3,7,11]:
            #     # x_v, x_i = self.tbsi_layers[tbsi_index](x_v, x_i, lens_z)
            #     x_single_rgb = self.cnn__.CT_fusion_rgb[tbsi_index](x_single[:, :self.num_zx, :], x_rbg)
            #     x_single_tir = self.cnn__.CT_fusion_tir[tbsi_index](x_single[:, self.num_zx:, :], x_tir)
            #     x_single = torch.cat([x_single_rgb, x_single_tir], dim=1)
            #     tbsi_index += 1
